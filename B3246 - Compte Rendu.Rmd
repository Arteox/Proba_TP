...
```
``` {r}
plot(mt[1:(Nsimu-1),1],mt[2:Nsimu,1],xlab='VN(i)', ylab='VN(i+1)', main='Mersenne Twister')
```
-- Commentaire : 
...
```
``` {r}
plot(rnd[1:(Nsimu-1),1],rnd[2:Nsimu,1],xlab='VN(i)', ylab='VN(i+1)', main='Randu')
```
-- Commentaire : 
...
```
``` {r}
plot(std[1:(Nsimu-1),1],std[2:Nsimu,1],xlab='VN(i)', ylab='VN(i+1)', main='Standard Minimal')
```
-- Commentaire : 
...
Fonction utilisée | Nombre de bits considérés | Fréquence obtenue | Observation du test
-------------| ------- | ------------- | -------------
Von Neumann   |   14 |0.0082607 | 
Mersenne Twister      | 32 | 0.49102 | 
Randu    | 31 |  0.19066 | 
Standard Minimal   | 31   | 0.46116 |
